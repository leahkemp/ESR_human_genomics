# Bioinformatics tools and software

A collection of bioinformatic tools and software we either developed internally or that were developed externally that we use in our analyses

## Table of contents

- [Bioinformatic tools and software](#bioinformatic-tools-and-software)
  - [Table of contents](#table-of-contents)
  - [Developed internally](#developed-internally)
  - [Developed externally](#developed-externally)
  - [For Illumina data](#for-illumina-data)
    - [RNAseq](#rnaseq)
    - [small RNAseq](#small-rnaseq)
  - [For Nanopore data](#for-nanopore-data)

## Developed internally

- [human_genomics_pipeline](https://github.com/ESR-NZ/human_genomics_pipeline)
- [vcf_annotation_pipeline](https://github.com/ESR-NZ/vcf_annotation_pipeline)
- [smncrna_analysis_template](https://github.com/leahkemp/smncrna_analysis_template)
- [rna_analysis_template](https://github.com/leahkemp/rna_analysis_template)
- [ESR_reporting_template](https://github.com/ESR-NZ/ESR_reporting_template)

## Developed externally

- [NVIDIA clara parabricks](https://www.nvidia.com/en-us/clara/genomics/)
  
### For [Illumina](https://www.illumina.com/) data

- [rnaseq](https://github.com/nf-core/RNAseq)
- [smrnaseq](https://github.com/nf-core/smrnaseq/)

### For [Nanopore](https://nanoporetech.com/) data

- [Pore-C-Snakemake](https://github.com/nanoporetech/Pore-C-Snakemake)
- [ont_human_workflow](https://github.com/leahkemp/ont_human_workflow)
